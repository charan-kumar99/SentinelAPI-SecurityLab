...
string connectionString = builder.Configuration.GetConnectionString("DefaultConnection") 
    ?? "Host=localhost;Database=apisecuritylab;Username=postgres;Password=postgres";

builder.Services.AddDbContext<AppDbContext>(options =>
{
    if (useInMemory)
    {
        options.UseInMemoryDatabase("ApiSecurityLabDb");
    }
    else
    {
...
builder.Services.AddSingleton<JwtTokenSettings>(sp => new JwtTokenSettings
{
    ...
    Issuer = builder.Configuration["Jwt:Issuer"] ?? "ApiSecurityLab",
    Audience = builder.Configuration["Jwt:Audience"] ?? "ApiSecurityLabUsers",
